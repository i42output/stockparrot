...
Copyright(c) 2026 Claude AI, Leigh Johnston
...
// ─── Attack tables (defined in chess.hpp, initialised at startup) ─────────────

inline U64 KNIGHT_ATTACKS[64] = {};
inline U64 KING_ATTACKS[64] = {};
inline U64 PAWN_ATTACKS[2][64] = {};

// Magic bitboard structs (retained for future use)
struct Magic {
    U64  mask;
    U64  magic;
    U64* attacks;
...
struct Move {
    ...
    int  promo = NO_PIECE;  // NO_PIECE if not a promotion
    bool ep = false;
    bool castle = false;
...
struct Board {
    ...
    int mailbox[64] = {};  // piece type per square
    int mailboxColor[64] = {};
...
inline void Board::setFromFEN(const std::string& fen) {
    ...
    for (char c : pos) {
        ...
        int pidx = static_cast<int>(order.find(c));
        int piece = pidx % 6;
...
inline void ttStore(U64 hash, int depth, int score, TTFlag flag, Move best) {
    TT[hash % TT_SIZE] = { hash, depth, score, flag, best };
}

inline bool ttProbe(U64 hash, int depth, int alpha, int beta, int& score, Move& bestMove) {
...
inline int quiescence(Board& b, int alpha, int beta, SearchInfo& info) {
    info.nodes++;
    ...
    MoveList ml;
    generateMoves(b, ml, /*capturesOnly=*/true);
...
inline int alphaBeta(Board& b, int depth, int alpha, int beta, SearchInfo& info) {
    if (info.timeUp()) return 0;
    info.nodes++;
...
// Moves within this many centipawns of the best score are considered equivalent
// at the root and one is chosen at random, adding variety without sacrificing
// meaningful strength.
inline constexpr int VARIETY_MARGIN = 15;

inline Move searchBestMove(Board& b, int timeLimitMs, int maxDepth = MAX_DEPTH) {
    SearchInfo info;
    info.startTime = std::chrono::steady_clock::now();
    info.timeLimit = timeLimitMs;

    // Track (score, move) for every legal root move from the last completed iteration
    ...
    for (int depth = 1; depth <= maxDepth; depth++) {
        ...
        if (!ttProbe(b.hash, depth, -INF, INF, score, ttMove)) ttMove = bestMove;
        ...
        std::vector<std::pair<int, Move>> currentRootMoves;
        ...
        for (int i = 0; i < ml.count; i++) {
            ...
            currentRootMoves.push_back({ s, ml.moves[i] });
            if (s > alpha) alpha = s;
        }

        if (!currentRootMoves.empty()) {
            rootMoves = currentRootMoves;
            bestScore = alpha;
    ...
    // Pick randomly among all root moves within VARIETY_MARGIN cp of the best
...
inline Move parseMove(const Board& b, const std::string& moveStr) {
    MoveList ml;
    generateMoves(b, ml);
    for (int i = 0; i < ml.count; i++) {
